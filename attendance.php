...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['time_in'])) {
        ...
        if ($attendance && $attendance['time_out'] === null) {
            ...
            $update_sql = "
                UPDATE attendance 
                SET time_out = ?, hours_worked = ?, overtime_hours = ?, night_hours = ?, night_overtime_hours = ? 
                WHERE employee_id = ? AND date = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("sdddiss", $time_now, $hours_worked, $overtime_hours, $night_hours, $night_overtime_hours, $user_id, $date);
            $update_stmt->execute();
